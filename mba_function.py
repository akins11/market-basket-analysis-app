...
def create_data_info(df, info_type, round_val = None):
    """
    parameter
    ---------
    df [pd.DataFrame] Transaction dataframe.
    info_type [string] The type of info to return. any of "no_transaction", "no_unique_customers",
                       "total_sales", "average_sales", "unique_products".
    round_val [integer] Round output value.
...
def most_purchased_products(df, output_type = None, max_unique_value=10, seq_color = seq_selected_color,
                            bg_color = plot_bg_color, font_color = ft_color):
    """
    parameter
    ---------
    df [pd.DataFrame]
    output_type [string] The type of output to return. Either a 'table' or a 'plot'.
    max_unique_value [integer] The number of unique product values to display.
    p_color [string] Bar colors.
    ...
    if output_type == "plot":
        ...
        f_fig.update_xaxes(color = font_color, showgrid = True, gridwidth = 1, gridcolor = "#E066FF")
        f_fig.update_traces(
...
def most_profitable_product(df, agg_function = "sum", output_type = None, max_unique_value = 10, seq_color = seq_selected_color,
                            bg_color = plot_bg_color, font_color = ft_color):
    """
    parameter
    ---------
...
def product_quantity(df, agg_function = "mean", output_type = None, max_unique_value = 10,
                     seq_color = seq_selected_color, bg_color = plot_bg_color, font_color = ft_color):
    """
    parameter
    ---------
    df [pd.DataFrame]
    agg_function [string] The type of aggregate function. any of "sum", "mean", "median", "min", "max".
    output_type [string] The type of output to return. Either a 'table' or a 'plot'.
    max_unique_value [integer] The number of unique product values to display.
    p_color [string] Bar colors.
...
def create_association_rule(df, min_support = 0.005, max_length = None, rule_metric = "lift", min_threshold = 0.7, output_type = "rules"):
    """
    parameter
    ---------
    min_support [float] The minimum apriori support 
    max_length [integer] Maximum length of the itemsets generated. passsed to apriori.
    rule_metric [string] Metric to evaluate if a rule is of interest. can be any of "support", "confidence", "lift", 
                         "leverage", "conviction".
    min_threshold [float] Minimal threshold for the evaluation metric.
    output_type [string] The type of table to return. either "rules" table or "sup_len"(support length) table.
...
def filter_products_contain(df, search_type = "any",
                            f_rule_type = None, f_product_type = None, 
                            s_rule_type = None, s_product_type = None, 
                            bitwise_opt = None):
    """
    parameter
    ---------
    df [pd.Dataframe|Association Rule]
    search_type [string] The way to search if value is 'any' then any row with any of the values supplied will be returned
    else if value is "all" then only rows that have all supplied values will be returned
    f_rule_type,s_rule_type [string] Either 'antecedents' or 'consequents'.
    f_product_type,s_product_type [list|string] The type of product(s) to search for.
    bitwise_opt [string] A bitwise operator. Either '|' or '&'.
...
def within_range_values(df, var_dict):
    """
    df [pd.DataFrame|Association Rule]
    var_dict [dict] A dictionary with ... variables.
    """
    valid_range = []
    ...
    for key, value, index in zip(var_dict.keys(), var_dict.values(), range(len(var_dict))):
        if value < valid_range[index][0] or value > valid_range[index][1]:
            raise ValueError(f"{key} values ({value}) is out of range. valid range is {valid_range[index][0]}  -  {valid_range[index][1]}")


def get_query_values(metric, metric_value, comp_op, bitw_op = None):
    """
    parameter
    ---------
    metric [list[string]] A list of metric names.
    metric_value [list[float]] A list of metric values.
    comp_op [list] A list of comparison operators.
    bitw_op [lsit] A list of bitwise operators.
    ...
    def remove_value(none_index, pop_list):
        if len(none_index) == 1:
    ...
    def get_selected_duplicate_index(dup_value, metrics):
        invalid_value_index = []
    ...
    def in_index(supplied_index, list_value):
        list_index = []
    ...
    def get_valid_values(metric_c, metric_value_c, comp_op, bitw_op = bitw_op):
...
def filter_rules_values(df, query_values, comp_op, bitw_op):
    """
    parameter
    ---------
    df [pd.DataFrame|Association Rule] 
...
def filter_rules_length(df, rule_type, comp_op, length):
    """
    parameter
    ---------
    df [pd.DataFrame|Association Rule]
    rule_type [string] The type of ...., can either 'antecedents' or 'consequents'.
    comp_op [string] A comparison operators.
    length [int] The amount of products.
...
def str_frozenset(df, df_type = "with_rules"):
    """
    parameter
    ---------
    df [pd.DataFrame]
...
def extract_product_rules(df, rule_type, rule_range = None, by_row = False):
    """
    parameters
    ----------
    df [pd.DataFrame]
    rule_type [string] The type of ...., can either 'antecedents' or 'consequents'.
    rule_range [int] A number to use as the maximum range of rows.
    by_row [bool] Whether to extract products by row or by combining them.
...
def get_protential_customer_product(df, ant_products = None, con_products = None, just_customer_id = False):
    """
    df [pd.DataFrame]
    ...
    just_customer_id [bool] Whether to return just the customer id that meet all conditions.
    ...
    if ant_products == [] or con_products == []:
        ...
        return DataFrame({"Problem": [f"{rule_name} returned empty products, make sure all avaliable options have valid inputs"]})
    ...
    if not isinstance(ant_products, list):
        ...
        for cus_id in list(valid_customer_df["Customer_ID"].unique()):
            get_customer_products = list(valid_customer_df.loc[valid_customer_df["Customer_ID"] == cus_id][product].values)
...
def protential_customer_product(df, ant_products = None, con_products = None, just_customer_id = False, distinct_product_group = False):
    """
    parameter
    ---------
    df [pd.DataFrame]
    ...
    con_products [list|string] Selected product(s) from the consequents column.
    just_customer_id [bool] Whether to return just the customer id that meet all conditions.
    distinct_product_group [bool] If True, each product will be treated as a single potential product.
...
def make_Product_Taxonomy(df, threshold = 60):
    """
    parameter
    ---------
    df [pd.DataFrame|Association rules]
    threshold [integer] Every unique product sku count that fall below the threshold will be lumped together as 'Others'.
    
    return
    -------
    A pandas data frame with a .......
    """
    f_tbl = df.copy()
    ...
    for product in f_tbl["Product"].unique():
        ff = f_tbl.loc[f_tbl["Product"] == product]["Unique_Product"].value_counts() < 60
        ...
        if len(ff.unique()) == 2:
            ...
            f_tbl.loc[f_tbl["Unique_Product"].isin(valid_products), "Product_Taxonomy"] = f_tbl.loc[f_tbl["Unique_Product"].isin(valid_products)]["Unique_Product"]
...
def rules_relationship(df, x_var, y_var, z_var = None, opacity = None, font_color = ft_color, bg_color = plot_bg_color,):
    """
    parameter
    ---------
    df [pd.DataFrame] A dataframe with association rules.
    x_var,y_var,z_var [string] A variable from the dataframe `df`. z_var is optional.
    opacity [float] from 0 to 1,  the boldness of each points in the plot.

    value
    -----
    A plotly.graph_objects.Figure object.
    """
    valid_variables = ["support", "confidence", "lift", "leverage", "conviction", "antecedent support",
                       "consequent support"]
    ...
    def plotly_scatter(labs, title, hover_data, f_d_color=None, f_c_color=None, z_var=z_var):
        f_plt = scatter(
            data_frame = df,
            x = x_var,
            y = y_var,
    ...
    f_fig.update_layout(plot_bgcolor = bg_color, paper_bgcolor = bg_color, title_font_color = font_color)
    f_fig.update_yaxes(color = font_color, showgrid = True, gridwidth = 1, gridcolor = "#E066FF")
    f_fig.update_xaxes(color = font_color, showgrid = True, gridwidth = 1, gridcolor = "#E066FF")
    return f_fig
...
def unique_products(df):
    ...
    parameter
    ---------
    df [pd.DataFrame] With either a variable called 'Product' or 'Product_Taxonomy'.
...
def metric_description(df, return_type = "rules"):
    """
    parameter
    ---------
    df [pd.DataFrame]
