...
def create_dataframe(df, page_size = 10, align_text = "left", precision = 2, increase_col_width = None, tbl_height = None,
    ...
    if change_tbl_color is not None:
        table_colors = increase_col_width
    else:
        table_colors = [
            # Header
...
def add_filter_div(id_type, children_div, *args):
    if ctx.triggered_id is not None:
        ...
        if isinstance(button_id, dict):
            children_div = [div for div in children_div if f"'type': '{button_id['type']}', 'index': {button_id['index']}" not in str(div)]
    else:
                        ...
                        [
                            dbc.DropdownMenu(
                                [
                                    ...
                                    dbc.DropdownMenuItem("Antecedent Support", id = f"{id_type}_ant_support_add", n_clicks = 0),
                                    dbc.DropdownMenuItem("Consequent Support", id = f"{id_type}_con_support_add", n_clicks = 0),
                                ],
                                label = "Add Query",
                                direction = "up"
                            ),
